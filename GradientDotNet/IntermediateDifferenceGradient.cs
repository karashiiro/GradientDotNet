namespace GradientDotNet
{
    public class IntermediateDifferenceGradient
    {
        ...
        public static void CalculateX(IImage src, IImage dst)
        {
            ...
            {
                // Calculate gradient for the first column
                dst.SetPixel(r, 0, src.GetPixel(r, 1) - src.GetPixel(r, 0));

                // Calculate gradient for the middle columns
                for (var c = 1; c < cols - 1; c++)
                {
                    dst.SetPixel(r, c, src.GetPixel(r, c + 1) - src.GetPixel(r, c));
                }

                // Calculate gradient for the last column
                dst.SetPixel(r, cols - 1, src.GetPixel(r, cols - 1) - src.GetPixel(r, cols - 2));
            }
        }
        ...
        public static void CalculateY(IImage src, IImage dst)
        {
            ...
            for (var c = 0; c < cols; c++)
            {
                dst.SetPixel(0, c, src.GetPixel(0, c + 1) - src.GetPixel(0, c));
            }
            ...
            for (var r = 1; r < rows - 1; r++)
            {
                for (var c = 0; c < cols; c++)
                {
                    dst.SetPixel(r, c, src.GetPixel(r + 1, c) - src.GetPixel(r, c));
                }
            }
            ...
            for (var c = 0; c < cols; c++)
            {
                dst.SetPixel(rows - 1, c, src.GetPixel(rows - 1, c) - src.GetPixel(rows - 2, c));
            }
        }
    }
}